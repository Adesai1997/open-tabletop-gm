...
here rather than hardcoding ~/eberron-dnd/. Set GM_CAMPAIGN_ROOT to move your
data anywhere — iCloud, Dropbox, network share, etc. Defaults to ~/eberron-dnd.

(The legacy env var DND_CAMPAIGN_ROOT is still honored for compatibility with the
upstream claude-dnd-skill, but GM_CAMPAIGN_ROOT takes precedence.)

Usage:
    from paths import campaigns_dir, characters_dir, campaign_dir, find_campaign

Environment:
    GM_CAMPAIGN_ROOT    Root of campaign data tree. Default: ~/eberron-dnd
                        Example: export GM_CAMPAIGN_ROOT=~/Dropbox/eberron-dnd
    DND_CAMPAIGN_ROOT   Legacy alias, honored if GM_CAMPAIGN_ROOT is unset.
...
Two distinct roots:
    * DATA root  — where campaigns/characters live (GM_CAMPAIGN_ROOT). User data.
...
_DEFAULT_ROOT = pathlib.Path("~/eberron-dnd").expanduser()


def _root() -> pathlib.Path:
    """Return the configured data root, expanded and absolute.

    GM_CAMPAIGN_ROOT takes precedence; DND_CAMPAIGN_ROOT is a legacy fallback.
    """
    raw = os.environ.get("GM_CAMPAIGN_ROOT", "") or os.environ.get("DND_CAMPAIGN_ROOT", "")
    if raw.strip():
...
def display_dir() -> pathlib.Path:
    """Deprecated no-op stub. This standalone build ships NO display companion.

    Retained only so any legacy import of `display_dir` resolves without error;
    the returned path does not exist and nothing should be written to it.
    """
    return skill_root() / "_no_display"
...
def runtime_dir() -> pathlib.Path:
    ...
    raw = os.environ.get("GM_RUNTIME_DIR", "").strip() or os.environ.get("DND_RUNTIME_DIR", "").strip()
...
def find_campaign(name: str) -> pathlib.Path:
    ...
    custom_root = (os.environ.get("GM_CAMPAIGN_ROOT", "").strip()
                   or os.environ.get("DND_CAMPAIGN_ROOT", "").strip())
    if not custom_root:
