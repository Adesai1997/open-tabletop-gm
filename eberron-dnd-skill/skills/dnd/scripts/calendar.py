...
# This standalone build ships no display companion; date announcements are
# returned to the DM inline rather than pushed to any external display.
SEND_PY = None
...
def _send_date(cal: dict) -> None:
    """No-op in the standalone build (no display companion).

    The formatted date is already printed to stdout by the calling command,
    so the DM sees it inline. Kept as a stub so callers need no changes.
    """
    return None
