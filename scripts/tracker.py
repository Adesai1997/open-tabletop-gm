...
def _push_conditions(entity_name: str, conditions):
    ...
    try:
        subprocess.run([sys.executable, PUSH_STATS, "--player", entity_name, "--conditions", cond_str], capture_output=True, timeout=3)
    except Exception:
        pass
...
def _send_announce(msg: str):
    try:
        proc = subprocess.Popen([sys.executable, SEND_PY, "--dice"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        proc.communicate(input=msg.encode(), timeout=3)
    except Exception:
        pass
